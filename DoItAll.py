...
def cropAndOCR(im,rangeTuple,formatType,argv,n_blocks):
	...
	# names_lst,boxesWithErrors = tessBox(box_lst)
        results = pool.map(tessBox,box_lst_divided)#,[pt]*n_blocks,[time]*n_blocks,[re]*n_blocks,[transliterate]*n_blocks,[sanscript]*n_blocks)
...
x`	n_blocks: Number of blocks the box_lst must be divided. Basically number of concurrent processes.
    ...
    """
    startPage = time.time()
    # cmd = 'convert -density 300 '+pdfFile+'['+str(rangeTuple[0])+'-'+str(rangeTuple[1])+'] -depth 8 '+'temp.tiff' #UBUNTU
...
def chunks(l, n): #Took from :https://stackoverflow.com/a/1751478/5345646
    ...
    for i in range(0, len(l), n):
        yield l[i:i + n]
def doItAll(basePDFName,outputCSV,totalPDFs,formatType,argv,n_blocks):
	gen_names_lst = []
	...
	for i in range(totalPDFs):
		...
		names_lst = mainProcess(pdfName,(0,PdfFileReader(open(pdfName,'rb')).getNumPages()-1),formatType,argv,n_blocks)
		gen_names_lst+=names_lst
